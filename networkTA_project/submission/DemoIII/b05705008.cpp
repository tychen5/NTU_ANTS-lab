...
void *connection_handler(void *socket_desc)
{
	...
	send(sock,s0,sizeof(s0),0);          
		...
		if(check.assign(client_message,0,9)=="REGISTER#"){
		...
		else if(check.assign(client_message,0,5) =="CHAT#"){
			...
			for(int i=5; i<read_size;i++){
				if(client_message[i] == '#'){
					//for(int k = i+1;k<read_size;k++){
						if(client_message[i]=='#'){
							required.assign(client_message,5,(i+1)-5-1);
							j = i;
							break;
						}
					//}		
			...
			money.assign(client_message,j+1,read_size-j-1);
			
			for(int l=0;l<100;l++){
				if(info[l].name==required){
					stringstream mon0;
					mon0 << info[l].balance;
					int mon1;
					mon0 >> mon1;
