...
void *service(void *socket_data){
    ...
    while(true){
		...
		// pay
		size_t position2 = request_type.find("#", position + 1, 1);
		if (position2 != string::npos){
			string payAmount = request_type.substr(position+1, position2);
			istringstream pA(payAmount);
			int intAmount;
			pA >> intAmount;
			if (intAmount >= account_balance[thread_self]){
				char reply[RQT_SIZE] = "211 FAIL\n";
    			send(sock, reply, (int)strlen(reply), 0);
    			continue;
			}
			
			string payAccountName = request_type.substr(position2+1, RQT_SIZE);
			bool paidExist = false;
			for (int name = 0; name < THREAD_LIMIT; name++){
				if (payAccountName == client_name[name]){
					
					account_balance[thread_self] -= intAmount;
					account_balance[name] += intAmount;
					char reply[RQT_SIZE] = "100 OK\n";
    				send(sock, reply, (int)strlen(reply), 0);
    				paidExist = true;
					break;
